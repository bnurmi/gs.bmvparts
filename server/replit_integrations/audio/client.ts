...
import { loggedChatCompletion, loggedStreamingChatCompletion, logUsageDirectly } from "../../openai-logger";
...
export async function voiceChat(
  ...
  const response = await loggedChatCompletion(openai, "chat-voice", {
    model: "gpt-audio",
...
export async function voiceChatStream(
  ...
  const audioBase64 = audioBuffer.toString("base64");
  const rawStream = await loggedStreamingChatCompletion(openai, "chat-voice", {
    model: "gpt-audio",
    modalities: ["text", "audio"],
...
export async function textToSpeech(
  ...
  const response = await loggedChatCompletion(openai, "tts", {
    model: "gpt-audio",
    modalities: ["text", "audio"],
    audio: { voice, format },
    messages: [
...
export async function textToSpeechStream(
  ...
  const rawStream = await loggedStreamingChatCompletion(openai, "tts", {
    model: "gpt-audio",
    modalities: ["text", "audio"],
    audio: { voice, format: "pcm16" },
    messages: [
...
export async function speechToText(
  ...
  // Transcription API does not return token counts; log with 0 tokens
  void logUsageDirectly("stt", "gpt-4o-mini-transcribe", 0, 0);
...
export async function speechToTextStream(
  ...
  // Log call — transcription API has no token counts
  void logUsageDirectly("stt", "gpt-4o-mini-transcribe", 0, 0);
